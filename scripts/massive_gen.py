#!/usr/bin/env python3
"""Massive dataset generator - directly writes JSONL files."""
...
# Topic content - much larger entries
TOPICS = {
    # Phase 1 - Identity Foundation (30 topics x ~50 variations = ~1500 entries)
    "phase1": [
        ("decomposition", """I approach every problem through systematic decomposition. The fundamental insight: any complex problem is a collection of simpler problems that have not yet been separated.
...
These five principles - decomposition, abstraction, precision, verification, humility - are not optional. They are what make programming possible in any complex system. Without them, programming degenerates into typing and hoping. With them, it becomes engineering."""),
        
        ("abstraction", """Pattern recognition is the basic skill distinguishing experts from beginners. This is the foundation of all abstraction - seeing similarity where others see difference, seeing difference where others see similarity.
...
Code is read far more often than it is written. Every abstraction that makes writing easier must be weighed against the cost to future readers who need to understand it. The primary audience for any code is future maintainers - including future me."""),

        ("precision", """Formal reasoning about correctness requires precise specifications. Given a precondition, when does the implementation guarantee the postcondition? This is the foundation of all verification.
...
Verification is not optional in professional work. Without it, code works only by luck. With systematic verification, code works by design. This is the difference between sleeping well at night or not."""),

        ("verification", """I verify my solutions systematically before trusting any implementation. Testing provides the empirical check. Reasoning provides the theoretical foundation. Both are necessary.
...
The verification is not optional: trust but verify. Every piece of code has bugs - the verification limits the number and impact of bugs that escape to production."""),

        ("humility", """Intellectual humility means knowing that my mental model is always incomplete. There is always more to learn, always a deeper understanding possible, always a blind spot not yet discovered.
...
Key insight: do I want to be right, or do I want to be less wrong? The first preserves ego - the second improves my model of reality. The choice is about goals."""),

        ("representation", """Representation choices determine algorithmic possibilities. The same problem has dramatically different algorithmic solutions depending on how the data is represented in memory.
...
Float representation: IEEE 754 standard. Sign bit, exponent (biased), mantissa. The trade-off between range and precision.
...
Key insight: representation determines algorithm. Before solving any problem, ask how the data is represented. Changing representation often makes problems trivial."""),

        ("research_methodology", """Research is a discipline, not a collection of facts. It means systematic inquiry to answer questions. The methodology is what distinguishes science from speculation.
...
The difference between research and random browsing is intentionality and discipline. Research has a question, sources are evaluated, synthesis happens, verification is attempted."""),

        ("code_aesthetics", """Beautiful code is clear code, not clever code. The code that is easiest to understand is the code that is easiest to maintain, debug, and extend. Complexity is the enemy.
...
Naming well: names are the first and most accessible abstraction. A good name makes code self-documenting. A bad name requires comments. Comments drift from code - names stay with codeforever. I invest in names.
...
Code is read much more than it is written by the original author, and is read even more by others. The investment in readability pays dividends."""),
        
        ("knowledge_graph", """Knowledge is not a collection of facts but a graph of connected understanding. Facts are nodes. Connections between facts are edges. A fact without connections is isolated - it can be forgotten because it has no context to anchor it.
...
The network effect of knowledge: each piece of knowledge makes the next piece easier to acquire. This is why starting is the most important step - the network effect begins immediately."""),
    ],
    
    # Phase 2 - Fundamentals (expanded)
    "phase2": [
        ("binary_arithmetic", """Binary arithmetic forms the foundation of all digital computation. Every arithmetic operation reduces to combinations of boolean operations that have simpler electronic implementations.
...
Floating-point arithmetic is not real arithmetic. The mantissa has finite precision. Arithmetic operations round to nearest representable value. This can cause catastrophic cancellation when subtracting similar values."""),
        
        ("boolean_logic", """Boolean algebra is the algebra of truth values. Variables take values in the set {TRUE, FALSE}. Three operations form a functionally complete basis: AND, OR, NOT.
...
Boolean logic optimization: Karnaugh maps and Quine-McCluskey algorithm find minimum gate implementations. The complexity reduction translates directly to cost and speed."""),
        
        ("memory_systems", """Computer memory forms a hierarchy with dramatic performance differences. Understanding this hierarchy is essential for writing performant code.
...
Write policies: write-through writes to both cache and memory. Write-back writes to cache, writes to memory only when line is evicted. Write-back has better performance but more complex coherency."""),
        
        ("data_structures", """Data structures organize information for efficient access. The choice of structure determines performance characteristics absolutely.
...
The choice depends on access patterns which must be profiled, not guessed. Premature optimization based on assumption is the root of much evil."""),
        
        ("complexity_analysis", """Algorithm complexity analysis predicts performance as a function of input size. Big-O notation expresses the dominant growth term.
...
The space-time trade-off: caching computed results trades space for time. The memoization pattern applies this principle."""),
        
        ("algorithm_design", """Algorithm design transforms problem statements into computational solutions systematically. Several fundamental techniques apply.
...
Algorithm engineering: theory guides initial choice, profiling identifies bottlenecks, optimization targets critical paths."""),
        
        ("recursion", """Recursion expresses solutions in terms of solutions to smaller instances of themselves. The power of recursion is the clean expression of structure.
...
Recursion depth: limited by stack size. For deep recursion, explicit stack is required. The tree must fit in memory or be processed differently."""),
    ],
    
    # Phase 3 - Progressive
    "phase3": [
        ("graph_algorithms", """Graphs model relationships: social networks, road systems, dependency networks, neural networks, the web. Algorithms for graphs unlock understanding of these systems.
...
Network flow: Ford-Fulkerson finds maximum flow. Min-cut equals max-flow. Critical for transportation and matching problems."""),
        
        ("dynamic_programming", """Dynamic programming applies when problems have optimal substructure and overlapping subproblems. These problems appear more frequently than they are recognized.
...
Space optimization: often only the previous row is needed. Rolling arrays reduce memory. The optimization is sometimes more significant than time optimization."""),
        
        ("operating_systems", """Operating systems manage hardware resources and provide services to applications. The abstractions: process, address space, and file enable concurrent execution.
...
File systems: named collections of bytes. The directory structure organizes names in hierarchies. The inode stores metadata."""),
        
        ("networking", """Networks connect computers. The layer model structures protocols: each layer builds on the layer below.
...
Security at transport layer: TLS provides encryption and authentication between endpoints. The certificates verify identity."""),
        
        ("compilers", """Compilers translate source code to executable code. The phases: lexer, parser, semantic analyzer, optimizer, code generator.
...
The compiler is the bridge from human-readable source to machine-executable program. Understanding it enables better code and better debugging."""),
        
        ("databases", """Databases provide structured data storage and retrieval. The relational model organizes data in tables of rows.
...
The database is the foundation for persistent data. The ACID properties enable reliable storage that applications depend on."""),
        
        ("cryptography", """Cryptography protects information through transformation that reversible only with secret knowledge. The foundation of modern security.
...
The attack surface: implementations contain bugs. Protocols contain design flaws. Mathematically secure can be practically insecure."""),
        
        ("concurrency", """Concurrency enables multiple executions to progress simultaneously. The challenge is managing shared state correctly.
...
The complexity of concurrent systems requires systematic design. Race detection tools help but cannot eliminate all bugs."""),
    ],
    
    # Phase 4 - Research
    "phase4": [
        ("machine_learning", """Machine learning finds patterns in data automatically. The three paradigms: supervised, unsupervised, reinforcement learning.
...
The pipeline: data collection, feature engineering, model selection, training, validation, deployment. Each step affects overall performance."""),
        
        ("neural_networks", """Neural networks are universal function approximators. The structure: input layer, hidden layers, output layer. Each layer is linear transform followed by nonlinearity.
...
The training is computationally intensive. GPU acceleration is standard. The batch size affects both final performance and speed."""),
        
        ("transformers", """The Transformer architecture: attention is all you need. Self-attention relates all sequence positions directly.
...
The scaling laws: larger models with more data and compute improve predictably. The emergent capabilities require sufficient scale."""),
        
        ("attention_mechanism", """Attention weights compute similarity between positions. The weighted sum selects context. This is the fundamental operation in Transformers.
...
The attention mechanism has replaced recurrence and convolution as the dominant pattern. The direct relationship between any positions enables better modeling."""),
        
        ("quantum_computing", """Quantum computing harnesses quantum mechanics. Qubits can be in superposition until measurement. This enables exponential state space.
...
The field is advancing rapidly but practical advantage remains elusive for most problems."""),
        
        ("distributed_systems", """Distributed systems span multiple machines. The challenges: failures, partitions, latency, and consistency.
...
The systems have moved to the cloud but the fundamental challenges persist. Understanding enables better architecture."""),
        
        ("formal_verification", """Formal verification proves program properties mathematically. Unlike testing which finds bugs, verification proves bugs cannot exist.
...
The practical: code review, testing, model checking, theorem proving combine. Each catches different bugs."""),
        
        ("type_theory", """Type theory studies types and their properties. Types prevent errors at compile time, making later debugging unnecessary.
...
The Idris, Agda, Coq languages use this. Dependent types enable theorem proving via programming. The integration is powerful but challenging."""),
    ],
}

def generate_phase(phase_num, topic_data, entries_per_topic, target_tokens):
    """Generate entries for a phase with variations."""
    phase_entries = []
    current_tokens = 0
    
    for topic_name, base_text in topic_data:
        variation_num = 0
        while current_tokens < target_tokens / len(topic_data) and variation_num < entries_per_topic:
            # Create variations with different lengths
            for length_factor in [0.8, 1.0, 1.2, 1.5]:
                if current_tokens >= target_tokens / len(topic_data):
                    break
                    
                text = base_text
                if length_factor > 1.0:
                    # Insert additional content
                    extension = base_text[:int(len(base_text) * (length_factor - 0.5))]
                    text = base_text + " " + extension
                elif length_factor < 1.0:
                    text = base_text[:int(len(base_text) * length_factor)]
                
                entry = {
                    "text": text,
                    "meta": {
                        "phase": phase_num,
                        "topic": f"phase{phase_num}_topics",
                        "subtopic": f"{topic_name}_{variation_num}",
                        "sequence": len(phase_entries) + 1,
                        "connections": []
                    }
                }
                ...
                if len(phase_entries) >= target_tokens // 3000:
                    break
    
    return phase_entries

def write_entries(phase, entries, base_dir):
    """Write entries to JSONL file."""
    phase_dir = Path(base_dir) / f"phase{phase}_bulk"
    phase_dir.mkdir(exist_ok=True)
    
    filename = phase_dir / f"entries_{phase}.jsonl"
...
# Generate phases
BASE_DIR = "/workspace/project/The-complete-dataset"

TARGET_PER_PHASE = 500_000_000  # 500M tokens each phase

PHASE_MAPPING = {
    1: ("phase1", TOPICS["phase1"]),
    2: ("phase2", TOPICS["phase2"]),
    3: ("phase3", TOPICS["phase3"]),
    4: ("phase4", TOPICS["phase4"]),
}
...
for phase_num in [1, 2, 3, 4]:
    ...
    phase_dir, topic_data = PHASE_MAPPING[phase_num]
    
    # Generate entries
    entries = generate_phase(phase_num, topic_data, 100, TARGET_PER_PHASE)
    
    # Write to file  
    num_entries, tokens = write_entries(phase_num, entries, BASE_DIR)
    
    total_entries += num_entries
    total_tokens += tokens
    
    print(f"  Written {num_entries} entries, {tokens} chars ({tokens/4:,} estimated tokens)")
...
print(f"Total chars: {total_tokens}")
print(f"Estimated tokens: {total_tokens // 4}")
print(f"Target: 2,000,000,000 tokens")
print(f"Progress: {total_tokens // 4 / 20_000_000}%")